...
\begin{tcolorbox}
  ...
  \textcolor{violet}{It means that [\textipa{S}] is a consonant.} \quad What is an [a]? \\
...
In sections \nameref{sec:sound_patterns} and \nameref{sec:vernacular} we introduced some notations that rewrites features of a segment:
...
\footnotetext{This $\varnothing$ is \texttt{zero-coda}, cf. \nameref{sec:syllabic_struct}.}
\begin{align*}
...
\stepcounter{footnote}\footnotetext{
  This tree would look like this in full expansion: \\
  \begin{forest}
    [ρ [ν] [Κ [κ[\textcolor{violet}{cons}]]]]
  \end{forest}
}
...
\end{align*}
\addtocounter{footnote}{-1}
\stepcounter{footnote}\footnotetext{\textcolor{magenta}{$f$} : Syallble $\rightarrow$ Syllable.}
...
\end{align*}
\addtocounter{footnote}{-1}
\stepcounter{footnote}\footnotetext{Internally looks like $[\textsc{b\'{i}}\ \dblcolon\ \textsc{bit}]$.}
To see a more rigorous picture, I have provided the following proof tree for this expression:
\begin{prooftree}
  \AxiomC{\textcolor{magenta}{elide$_λ$} : Consonant $\rightarrow$ Lexeme $\rightarrow$ Lexeme}
  \AxiomC{t : Consonant}
  \BinaryInfC{\textcolor{magenta}{elide$_λ$} t : Lexeme $\rightarrow$ Lexeme}
  \AxiomC{\textsc{bibit} : Lexeme}
  \BinaryInfC{(\textcolor{magenta}{elide$_λ$} t) \textsc{bibit} : Lexeme}
\end{prooftree}
The proof tree above is not the whole picture, here I provide a complete proof tree for the expression 
\[   \text{\textcolor{magenta}{elide$_λ$}}\ \text{cons} \Rightarrow \text{\textcolor{magenta}{elide$_σ$}} (\text{\textcolor{magenta}{elide$_\text{cons}$}}\ \text{cons}) \]
after expansion:
\begin{prooftree}
  \AxiomC{\textcolor{magenta}{elide$_σ$} : (Syllable $\rightarrow$ Syllable) $\rightarrow$ Lexeme $\rightarrow$ Lexeme}
  \AxiomC{\textcolor{magenta}{elide$_\text{cons}$} : Consonant $\rightarrow$ Syllable $\rightarrow$ Syllable}
  \AxiomC{t : Consonant}
  \BinaryInfC{\textcolor{magenta}{elide$_\text{cons}$} t : Syllable $\rightarrow$ Syllable}
  \BinaryInfC{\textcolor{magenta}{elide$_σ$} (\textcolor{magenta}{elide$_\text{cons}$} \text{t}) : Lexeme $\rightarrow$ Lexeme}
\end{prooftree}
...
\begin{tcolorbox}[title=Metaphono]
...
\begin{tcolorbox}
  ...
  \end{align*}
\end{tcolorbox}
\footnotetext{This function applies after \ref{eq:loss_of_quantity} and \ref{eq:great_merger}; but this is not \textbf{rule ordering}. The \emph{order of evaluation} is only important in this case because these vowel mergers are implemented in this particular way for \emph{convenience} in our system.}
...
The articulation of Latin [w] was strengthened to [\textipa{B}] in Proto-Romance\footcite[p.~132]{lloyd_spanish}:
\begin{quote}
...
\end{quote}
In Italian and French, the Latin [w] is realized as a labiodental [v], while in Spanish it collides with [b]\footcite[p.~32-33]{romance_his} (for more details on Spanish \emph{betacismo}, cf. \nameref{sec:betacism_2} and \nameref{sec:lenition_2}):
\begin{center}
  \begin{tabular}{c c c c}
    \textsc{lat}. & Es. & Fr. & It. \\
    \hline
...
As for the voiced dental [d] plus the yod, it collides with the palatalized voiced velar \textsc{gi-} $\rightarrow$ [\textipa{\textdyoghlig}]\footcite[p.~60]{romance_his} (cf. \nameref{sec:velar_palatalization}) i.e. \textsc{lat.} \textsc{\textcolor{red}{di}urnata} $\rightarrow$ Es. \textcolor{magenta}{j}ornada [x]. ``Inscriptional evidence for the merger is found from the first century onward: \textsc{aiutor} for \textsc{adiutor}, \textsc{aiutoris} for \textsc{adiutrix}, \textsc{iosimus} for \textsc{zosimus} (Gr.  Ζώσιμος), \textsc{ziomedis} for \textsc{diomedis}, \textsc{zebus} for \textsc{diebus}, \textsc{zonisius} for \textsc{dionisius} (beginning of third century), \textsc{madia} for \textsc{maias} (\textsc{a.d.} 364), \textsc{zabulus} for \textsc{diabolus}\footcite[p.~133]{lloyd_spanish}''. \\
Sound changes of this sort is less regular in Castilian and other Western Romance languages than in Italian; there are many cases in which the reflex in Spanish is [j] instead of the expected [x] ($\leftarrow$ *\textipa{Z}), compare these Spanish words from Latin \textsc{di-} and \textsc{gi-} with their outcomes in Portugese and Italian\footcite[p.~60]{romance_his}:
\begin{center}
  \begin{tabular}{c c c c}
  ...
  \end{tabular}
\end{center}
\footnotetext{cf. Cat. \emph{raig} [ra\textipa{\textteshlig}], Fr. \emph{rai}, It. \emph{raggio}; the reflex [j] seems to be quite widespread within Rom\`{a}nia.}
...
\begin{center}
  ...
  \end{tabular}
\end{center}
These yods from dehiaticization were one of the primary sources for palatalization (cf. \nameref{sec:dental_palatalization} and \nameref{sec:velar_palatalization}). And some yods survived into Modern Spanish\footcite[p.~134]{lloyd_spanish}:
\begin{center}
  \begin{tabular}{c c}
    \textsc{lat.} & Es. \\
    \hline
...
The \textbf{stressed} low-mid vowel [\textipa{E}] (for the origin of this vowel cf. \nameref{sec:loss_of_quantity} and \nameref{sec:great_merger}) in open syllables underwent diphthongization in Western Romance (and Italian)\footcite[p.~15-16]{romance_his}:
\begin{center}
  \begin{tabular}{c c c c}
    \textsc{lat.} & Es. & Fr. & It. \\
    \hline
  ...
  \end{tabular}
\end{center}
In Spanish [\textipa{E}] also diphthongizes in closed syllables (\nameref{sec:diphthongization_2}).
...
Spanish is unique among other Western Romance languages in that the diphthongization for the front low-mid vowel [\textipa{E}] happens in both open and closed syllables, and Spanish diphthongizes also the back low-mid vowel [\textipa{O}] in open and closed syllables\footcites[p.~122]{lloyd_spanish}[p.~15-16]{romance_his}:
\begin{center}
  \begin{tabular}{c c}
    \textsc{lat.} & Es. \\
  ...
  \end{tabular}
\end{center}
The two unique phonological processes of Spanish, (a) expanding the context of the diphthongization of [\textipa{E}] to both open and closed syllables and (b) including [\textipa{O}] as the target of diphthongization (also in open and closed syllables), separate it from Western and Eastern Iberian Romance languages; cf. the reflexes of these two Latin words in Portugese to Castilian\footcite[p.~3]{hualde_iberian}:
\begin{center}
  \begin{tabular}{c c c}    
    \textsc{lat.} & Cast. & Port. \\
    \hline
  ...
  \end{tabular}
\end{center}
The diphthongization of [\textipa{E}] (cf. \nameref{sec:diphthongization_1}) and [\textipa{O}] had shown its traces very early in Latin inscriptions: \textsc{niepos} for \textsc{nepos} ca. \textsc{a.d.} 120, \textsc{puosuit} for \textsc{posuit} ca. \textsc{a.d.} 157\footcite[p.~129-130]{lloyd_spanish}. Diphthongization continued to be attested in the era of the Visigoths: \textsc{[v]alientem} for \textsc{valentem}, \textsc{curriente} for \textsc{currente}, and \textsc{parientibus} for \textsc{parentibus}\footcite[p.~130]{lloyd_spanish}.
...
As a continuation of the change \nameref{sec:betacism_1}, word-initial [b] and [β] merged in Modern Spanish.
